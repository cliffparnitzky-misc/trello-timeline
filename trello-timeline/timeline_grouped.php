...
<?php $pageTitle = "Timeline (grouped by board)"; ?>
...
?>
<?php $boards = array(); ?>
<?php $boardIdCounter = 1; ?>
<?php foreach ($jsonBoardsOutput as $board) : ?>
	<?php if (!$board->closed): ?>
		<?php $boards[$board->id] = array('visId' => $boardIdCounter, 'name' => $board->name, 'hasCards' => false, 'obj' => $board); ?>
		<?php $boardIdCounter++; ?>
	<?php endif; ?>
<?php endforeach; ?>
<?php $jsonCardsUrl = "https://trello.com/1/boards/[BOARD_ID]/cards/open?key=" . TRELLO_KEY . "&token=" . TRELLO_TOKEN; ?>
...
<?php if (!$board->closed): ?>
	...
	<?php foreach ($jsonCardsOutput as $card) : ?>
		<?php if ($card->due != null): ?>
			<?php $boards[$board->id]['hasCards'] = true; ?>
	...
	'group'    : <?php echo $boards[$board->id]['visId']; ?>,
...
<?php if ($board['hasCards']): ?>
	{id: <?php echo $board['visId']; ?>, content: '<a href="timeline_single.php?board=<?php echo $board['obj']->id; ?>" target="_blank" title="Nur dieses Board in einem neuen Fenster anzeigen"><?php echo $board['name']; ?></a>'},
<?php endif; ?>
<?php endforeach; ?>
	]);
</script>
